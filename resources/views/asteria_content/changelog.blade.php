...
@section('title','Changelog') <!-- Sets the title of the page -->
...
<div class="header-icon">
   ...
   <h1 class="header-icon-text">Changelog</h1>
</div>
@endsection
...
<div class="changelog_header">
    ...
    <div>
        <h1 class="changelog_title">🌠Your project or product changes and updates!</h1> <!-- Changelog title -->
    </div>
        <div class="changelog_content">
            @foreach ($changelog as $changelogs) <!-- Loop through changelog entries -->
            <h1 class="changelog_h2">{{$changelogs['title']}}</h1>
            <p class="changelog_p_2">{{$changelogs['content']}}</p>
            @endforeach
        </div>
        <div class="changelog_pag">{{$changelog->links()}}</div> <!-- Pagination links -->
    </div> 
</div>
</section>
@endsection
