...
@section('title', 'Log up') <!-- Sets the title of the page -->
...
<div class="header-icon">
    ...
    <h1 class="header-icon-text">REGISTER</h1> <!-- Page header -->
</div>
@endsection
...
<div class="form-header">
   <form action="{{ route('asteria_register_check') }}" method="post"> <!-- Form for registration -->
      @csrf 

      <img class="form-icon" src="/img/user.svg" alt="register"> <!-- Icon for the form -->
      <h2 class="form-text">REGISTER</h2> <!-- Form header -->
      ...
      <input class="form-content-1" name="email" type="email" placeholder="Email Address" value="{{ old('email') }}"> <!-- Input field for email -->
      @if ($errors->has('email'))
      <div class="alert alert-danger" style="color:#ffffff; margin-top:3px;">{{ $errors->first('email') }}</div> <!-- Displays errors for email -->
      @endif
      ...
      <input class="form-content-1" name="password_confirmation" type="password" placeholder="Confirm Password"> <!-- Input field for password confirmation -->
      @if ($errors->has('password_confirmation'))
      <div class="alert alert-danger" style="color:#ffffff; margin-top:3px;">{{ $errors->first('password_confirmation') }}</div> <!-- Displays errors for password confirmation -->
      @endif

      <button type="submit" class="form-btn">REGISTER</button> <!-- Submit button for the form -->
      <a href="{{ route('asteria_login') }}" class="form-p">I already have an account</a> <!-- Link to the login page -->
   </form>
</div>
